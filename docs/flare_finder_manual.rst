...
TOFFEE allows you to determine how you want to detrend you lightcurve within our given function. You can turn it off, only have it
run through wotan, or additionally fun through a periodogram to pick up any residual periodicity. Additionally you can set what
quality flags to include for analysis and whether or not you want to clip points on either side of breaks.

If you are dialing TOFFEE detection for your own goals it's recommended that the varialbes magic_flare_std, sec_flare_std,
consecutive, fit_twice, and fit_multiple_secs are the only ones changed. 

The variables prim_marg_rate, rate_above_threshold, and sec_marg_rate have all been tuned for best results.
You can alter them but I advise setting rate_above_threshold no lower than 0.75.

There are also a host of arguments about how to color certain things if you turn on the in-built plotting function visualize_fit.
You don't really need to toy with them but if you're working in dark mode you may need to.

time: 1D array-like object representing the time coordinates of the lightcurve in BJD. The detrending utilizes the orbit timing so these should be
      in the default units from lightkurve.

flux: 1D array-like object representing photometric flux. Can either be in raw units of e/sec becuase we'll normalized later or can be normalized already.

flux_err: 1D array-like object representing photometric error on the cadences.
          Can either be in raw units of e/sec becuase we'll normalize later or can be normalized already.

quality: 1D array-like object representing the quality flags of the cadences from TESS. We utilize filtering to remove poor quality readings from the
